...
def main():
    ...
    logger.info(f"Saved CV input to: {out_path}")
    logger.info(f"Matrix shape: {Z.shape}")
    logger.info(f"Observed entries: {n_observed}")
    logger.info(f"Held-out entries: {n_holdout}")
    logger.info(f"Holdout fraction realized: {n_holdout / n_observed:.6f}")
